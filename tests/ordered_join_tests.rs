use f_ck::dsl::{QueryPlan, Source, DestinationField, PrimaryKeySpec, PrimaryKeyLogic, Mapping, MergePolicy, SourceFieldSpec};
...
fn test_primary_source_order_preservation() {
    ...
    let query = QueryPlan {
        ...
        primary_keys: PrimaryKeySpec {
            logic: PrimaryKeyLogic::Or,
            keys: vec!["customer_id".to_string()],
        },
        mappings: vec![
            Mapping {
                destination_field: "customer_id".to_string(),
                policy: MergePolicy::FirstMatch { priority: vec!["A".to_string(), "B".to_string()] },
                source_fields: vec![
                    SourceFieldSpec {
                        id: "cid_a".to_string(),
            ...
            Mapping {
                ...
                policy: MergePolicy::FirstMatch { priority: vec!["A".to_string()] },
                source_fields: vec![
                    SourceFieldSpec {
                        id: "name_a".to_string(),
                        source_file_id: "A".to_string(),
            ...
            Mapping {
                ...
                policy: MergePolicy::FirstMatch { priority: vec!["B".to_string()] },
                source_fields: vec![
                    SourceFieldSpec {
                        id: "email_b".to_string(),
                        source_file_id: "B".to_string(),
    ...
    let customer_ids: Vec<i64> = df.column("customer_id").unwrap()
        .i64().unwrap()
        .into_no_null_iter()
        .collect();
    ...
    let names: Vec<&str> = df.column("name").unwrap()
        .str().unwrap()
        .into_no_null_iter()
        .collect();
...
fn test_primary_source_priority_deduplication() {
    ...
    let query = QueryPlan {
        ...
        mappings: vec![
            ...
            Mapping {
                ...
                source_fields: vec![
                    SourceFieldSpec {
                        id: "amount_a".to_string(),
                        source_file_id: "A".to_string(),
                        column_name: "amount".to_string(),
                    },
                    SourceFieldSpec {
                        id: "amount_b".to_string(),
                        source_file_id: "B".to_string(),
    ...
    let order_ids: Vec<i64> = df.column("order_id").unwrap()
        .i64().unwrap()
        .into_no_null_iter()
        .collect();
    
    // Only orders from A should appear, in A's order
    assert_eq!(order_ids, vec![1001, 1002, 1003]);
    
    // For order 1002, should use value from A (200.75), not B (999.99)
    let amounts: Vec<f64> = df.column("amount").unwrap()
        .f64().unwrap()
        .into_no_null_iter()
        .collect();
    assert_eq!(amounts, vec![100.5, 200.75, 150.0]);
}
...
fn test_secondary_source_records_appended() {
    ...
    let product_ids: Vec<Option<i64>> = df.column("product_id").unwrap()
        .i64().unwrap()
        .into_iter()
        .collect();
...
fn test_three_way_join_ordering() {
    ...
    let query = QueryPlan {
        ...
        mappings: vec![
            Mapping {
                ...
                policy: MergePolicy::FirstMatch { priority: vec!["A".to_string(), "B".to_string(), "C".to_string()] },
                source_fields: vec![
                    SourceFieldSpec {
                        id: "uid_a".to_string(),
                        source_file_id: "A".to_string(),
            ...
            Mapping {
                ...
                policy: MergePolicy::FirstMatch { priority: vec!["C".to_string()] },
                source_fields: vec![
                    SourceFieldSpec {
                        id: "phone_c".to_string(),
                        source_file_id: "C".to_string(),
    ...
    let user_ids: Vec<i64> = df.column("user_id").unwrap()
        .i64().unwrap()
        .into_no_null_iter()
        .collect();
    ...
    let usernames: Vec<Option<&str>> = df.column("username").unwrap()
        .str().unwrap()
        .into_iter()
        .collect();
